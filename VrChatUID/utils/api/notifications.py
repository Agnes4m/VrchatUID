...
async def get_notification(client: ApiClient, notification_id: str):
    """获取指定通知详情"""
    api = NotificationsApi(client)
    return await asyncio.to_thread(api.get_notification, notification_id=notification_id)


async def accept_friend_request(client: ApiClient, notification_id: str):
    """接受好友请求"""
    api = NotificationsApi(client)
    return await asyncio.to_thread(api.accept_friend_request, notification_id=notification_id)


async def mark_notification_as_read(client: ApiClient, notification_id: str):
    """将通知标记为已读"""
    api = NotificationsApi(client)
    return await asyncio.to_thread(api.mark_notification_as_read, notification_id=notification_id)


async def delete_notification(client: ApiClient, notification_id: str):
    """删除通知"""
    api = NotificationsApi(client)
    return await asyncio.to_thread(api.delete_notification, notification_id=notification_id)
